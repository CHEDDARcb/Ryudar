...
この文書はRyudar プロジェクト全体を確認する前に、ソースコードの構成、主要クラスの役割、レンダリング処理の流れを把握するための説明資料です。
...
Ryudar は DirectX 11 / C++17 で作成されたリアルタイム 3D レンダラーです。
...
プロジェクトの大まかな構成は、DirectX 11 の初期化とフレームループを担当する `AppBase`、実際のシーンを構築する `Ryudar`、メッシュ描画を担当する `ClassicLit::MeshGroup`、環境マップを扱う `CubeMapping`、ポストプロセスを担当する `ImageFilter` を中心に組まれています。
...
実際の開発用ソースは、主に `Headers`、`Sources`、`Shaders`、`Assets` にあります。
...
`Ryudar` はこの `AppBase` を継承して、シーン固有の GUI、更新、描画処理を実装しています。
...
環境マップは `CubeMapping`、ポストプロセスは `ImageFilter` と `BuildFilters()` によって構成されます。

今後はプロジェクト更にゲームエンジンのような構成でリファクタリングし、PBRモデル適用する予定です。
