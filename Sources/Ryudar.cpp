...
void Ryudar::Update(float dt)
{
    ...
    //     projRow.Transpose());
    m_cubeMapping.UpdateConstantBuffers(
        m_device, m_context, viewRow.Transpose(),
        projRow.Transpose());
...
{ 
    ...
    auto combineFilter = make_shared<ImageFilter>(
...
void Ryudar::DrawRenderOptionsGUI(BasicMeshGroup &meshGroup)
{
    ...
    if (ImGui::SliderFloat(
                "Normal scale", 
                &meshGroup.m_normalVertexConstantData.scale, 0.0f, 1.0f)
       )
    {
