                                     ...
                                     const int numSlices, const int numStacks)
{
    ...
    for (int j = 0; j <= numStacks; j++)
    {
        ...
        Vector3 stackStartPoint = Vector3::Transform(
            leftBottom, Matrix::CreateTranslation(0.0f, dy * j, 0.0f));
        for (int i = 0; i <= numSlices; i++)
        {
            ...
            v.position = Vector3::Transform(
                stackStartPoint, 
                Matrix::CreateTranslation(dx * i, 0.0f, 0.0f));
...
MeshData GeometryGenerator::MakeCylinder(const float bottomRadius, 
    ...
    for (int i = 0; i <= sliceCount; i++)
    {
        ...
        v.position = Vector3::Transform(
            Vector3(bottomRadius, -0.5f * height, 0.0f),
                                       ...
                                       const int numStacks)
{
    ...
    for (int j = 0; j <= numStacks; j++)
    {
        ...
        for (int i = 0; i <= numSlices; i++)
        {
            ...
            v.normal.Normalize();
            v.texcoord = Vector2(float(i) / numSlices, 
                                 1.0f - float(j) / numStacks);
