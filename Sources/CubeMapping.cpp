...
{
    // .dds 파일 읽어들여서 초기화
    D3D11Utils::CreateCubemapTexture(
        device, diffuseFilename, m_diffuseResView);
    D3D11Utils::CreateCubemapTexture(
        device, specularFilename, m_specularResView);

    // 메쉬(버퍼) 생성
    m_cubeMesh = std::make_shared<Mesh>();
    // VertexConstantBuffer 생성
    D3D11Utils::CreateConstantBuffer(
        device, vertexConstantData, m_cubeMesh->vertexConstantBuffer);
    ...
    std::reverse(
        cubeMeshData.indices.begin(), cubeMeshData.indices.end());

    // VertexBuffer 생성
    D3D11Utils::CreateVertexBuffer(
        device, cubeMeshData.vertices, m_cubeMesh->vertexBuffer);
    // IndexBuffer 생성
    m_cubeMesh->m_indexCount = UINT(cubeMeshData.indices.size());
    D3D11Utils::CreateIndexBuffer(
        device, cubeMeshData.indices, m_cubeMesh->indexBuffer);
    ...
    D3D11Utils::CreateVertexShaderAndInputLayout(
        device, L"CubeMappingVertexShader.hlsl",
        basicInputElements, m_vertexShader, m_inputLayout);
    // PixelShader 설정.
    D3D11Utils::CreatePixelShader(
        device, L"CubeMappingPixelShader.hlsl", m_pixelShader);
    ...
    device->CreateSamplerState(
        &sampDesc, m_samplerState.GetAddressOf());
}
...
void CubeMapping::Render(ComPtr<ID3D11DeviceContext> &context)
{
    ...
    context->IASetVertexBuffers(
        0, 1, m_cubeMesh->vertexBuffer.GetAddressOf(), &stride, &offset);
    context->IASetIndexBuffer(
        m_cubeMesh->indexBuffer.Get(), DXGI_FORMAT_R32_UINT, 0);
